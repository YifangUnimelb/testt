...
All data is obtained from the RBA. Daily or monthly data is converted to quarterly format by averaging. Exchange rate, cash rate and unemployment rate are in their original format.Other variables will be log transformed. These are variables included in this paper:
...
pcpi <- ggplot(data_trans, aes(date,cpi_log)) + geom_line(size = 0.25)+labs(title = "CPI") +
  theme(plot.title = element_text(size = 10),
        axis.title.x = element_text(size = 8),
        axis.title.y = element_text(size = 8))

pgdp <- ggplot(data_trans, aes(date,gdp_log)) + geom_line(size = 0.25)+labs(title = "GDP") +
  theme(plot.title = element_text(size = 10),
        axis.title.x = element_text(size = 8),
        axis.title.y = element_text(size = 8))


pm1 <- ggplot(data_trans, aes(date,m1_log)) +geom_line(size = 0.25)+labs(title = "M1") +
  theme(plot.title = element_text(size = 10),
        axis.title.x = element_text(size = 8),
        axis.title.y = element_text(size = 8))
...
Therefore the kernel of the likelihood function:
...
Therefore the kernel of the prior distribution:
...
Collect corresponding terms and then we have the full conditional posterior distribution:
...
$$h_t = h_{t-1}+\eta_t$$
where $\eta_{t} \sim N(0, \sigma^{2}_{\eta})$ and $h_0 \sim N(0, \delta^2_{\eta})$, which implies $\epsilon_{t} \sim N(0,e^{h_t} \Sigma)$.
...
#| message: true
#| warning: false
plot(y1, type='l',ylim=c(min(y), max(y)), col= mcxs3, ylab='', xlab='', main='Bivariate Random Walk')
...
Sigma.b = bvar_est(Y_ts, X_ts,p)$Sigma.posterior.draws
...
for (s in 1:S) {
  for (i in 1:h) {
    ht_forecast[1,s] = bvarsv$H[132,i]
    sigma2v = bvarsv$sigma.v2[i+50]
