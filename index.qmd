...
Following @chan2020large, $\Omega$ will be defined as $diag(\lambda_1, \lambda_2,...,\lambda_T)$, where each lambda is independently drawn from an Inverse Gamma 2 distribution $IG2(\nu,\nu)$. The predictive density:
...
# Sigma posterior mean
round(apply(bvar_t(Y,X)$Sigma.posterior.draws[,,1:1000], 1:2, mean),2)*round(mean(bvar_t(Y,X)$lambda.posterior.draws[,1:1000]),2)
