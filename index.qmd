...
The following ACF plots of 400 times log difference CPI, real GDP, M1 monetary aggregate show autocorrelation over small number of lags, therefore, likely to be stationary. M3 monetary aggregate, cash rate and unemployment rate display a long memory pattern. They are likely non-stationary.
...
```
In PACF plots, significant correlations were observed in very large number of lags for log difference M1 and M3 money aggregate and unemployment rate.
```{r PACF}
#| echo: false
#| message: false
#| warning: false
...
The table shows the result of Augmented Dickey-Fuller test. I can reject the non-stationary null for log difference CPI, GDP, M1, cash rate only. Note that Cash rate display a long memory pattern in ACF plot, but tested stationary here. This needs further investigation.
...
adfcpilogd <- adf.test(data_trans$cpi_log)
adfdgplogd <- adf.test(data_trans$gdp_log)
adfm1logd <- adf.test(data_trans$m1_log)
adfm3logd <- adf.test(data_trans$m3_log)
adfcr <- adf.test(data_trans$cr)
adfun_em <- adf.test(data_trans$un_em)

p_value <- c(0.01796, 0.01, 0.02154, 0.1622, 0.01, 0.3884)
ADF_stats <- c(-3.8653, -5.765, -3.7918,-2.9958, -4.3135,-2.4522)
Variable <- c("CPI_log_dif", "GDP_log_dif","M1_log_dif","M3_log_dif","Cash rate","Unemployment")
...
In standard form, the error term is normally distributed 
$$\epsilon_t \sim iidN(0, \Sigma)$$ or 
$$E \sim MN(0, \Sigma \otimes I_T)$$

- $\Sigma$ is a $n \times n$ covariance matrix
- $I_T$ is an $T \times T$ identity matrix. 

In this paper, following Hartwig(2022), I use the specification:
...
- $\lambda_t \sim IG(\nu/2,\nu/2)$ and 
- $h_t = \rho h_{t-1}+ \epsilon^{h}_{t}$ with
- $|\rho|< 1$ and $\epsilon^{h}_{t} \sim N(0, \sigma^{2}_h)$.

In matrix form: 

$$E \sim MN(0,\Sigma \otimes \Omega)$$.
